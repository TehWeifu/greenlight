...
func (app *application) registerUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Validate the user struct and return the error message to the client if any of
	// the checks fail.
	...
	// Insert the suer data into the database.
	err = app.models.Users.Insert(user)
	if err != nil {
		switch {
		// If we get a ErrDuplicateEmail error, use the v.AddError() method manually
		// add a message to the validator instance, and the call our
	...
	// email.
	app.background(func() {
		// As there are now multiple palaces of data that we want to pass to our email
		// palpates we create a map to act as a 'holding structure' for the data. This
		...
		if err != nil {
			// Importantly, if there is an error sending the email then we sue the
...
func (app *application) updateUserPasswordHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Save the updated user record in out database, checking for any edit conflicts as
	// toml.
	err = app.models.Users.Update(user)
	if err != nil {
		switch {
