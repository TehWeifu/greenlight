...
// Implement a UnamrshalJSON() method on the Runtime type so that it satisfies the
// json.Unmarshaler interface. IMPORTANT: Because UnmarhsalJSON() need to modify the
// receiver (our Runtime type), we must use a pointer receiver for this to work
// correctly. Othersie, we will only be modifying a copy (which is then discarded when
// this method returns)
func (r *Runtime) UnmarshalJSON(jsonValue []byte) error {
	...
	// double-quotes from thsi string. If we can't unquote it, then we return the
	...
	// If it isn't, we return the ErrInvalidRuntime error again.
	...
	// Convert the int32 to a Runtime type and assign this to the receiver. Nto that we
